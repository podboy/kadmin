...
VERSION := $(shell python3 -c "from kadmin.attribute import __version__; print(__version__)")
...
pylint:
	pylint $(shell git ls-files kadmin/*.py)
flake8:
	flake8 kadmin --count --select=E9,F63,F7,F82 --show-source --statistics
	flake8 kadmin --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics
pytest:
	pytest --cov=kadmin --cov-report=term-missing --cov-report=xml --cov-report=html --cov-config=.coveragerc --cov-fail-under=100
pytest-clean:
	rm -rf .pytest_cache
